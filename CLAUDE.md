...
2. **슬라이드 내용에 충실.** 슬라이드에 없는 사실·수치·인용·출처를 지어내지 않는다.
   내용이 빈약하면 억지로 늘리지 말고 주어진 범위에서 풀어 쓴다.
...
4. **드라이한 서식.** docx는 본문 글꼴 하나(맑은 고딕 11pt), 장 제목만 약간 키움.
   색·표·테두리·머리글/꼬리글·장식 없음.
5. **분량.** A4 **35쪽 이상**(기본 목표 38, 하한 35 무조건 보장). `pages × chars_per_page`로
   목표 글자 수를 잡고, 부족하면 수렴 루프가 모든 장을 슬라이드 근거 안에서 확장해 35쪽을 채운다.
   슬라이드 내용이 적어도 35쪽을 채우므로 늘어짐이 없는지 사람 검토가 필요하다.
